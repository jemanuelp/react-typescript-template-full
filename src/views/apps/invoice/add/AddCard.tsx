...
import {Hash, Plus, X} from 'react-feather';
...
const AddCard = () => {
  ...
  const [selected, setSelected] = useState<ClientInterface | null>(null);
  const [picker, setPicker] = useState<Date[]>([new Date()]);
  const [invoiceNumber, setInvoiceNumber] = useState<number>();
  const [dueDatepicker, setDueDatePicker] = useState<Date[]>([new Date()]);
  ...
  // ** Invoice To OnChange
  const handleInvoiceToChange = (data: any) => {
    setValue(data);
    if (clients) {
      setSelected(clients.filter((i: ClientInterface) => i.name === data.value)[0]);
    }
  };
