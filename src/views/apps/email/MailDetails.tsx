...
import {LabelColor} from './interfaces/Colors';
import {paginateMail, updateMailLabel, updateMails} from './store';
import {IAttachment} from '../../../domains/interfaces/IAttachment';

export type MailDetailsProperty = {
  openMail: boolean;
  dispatch: any;
  mail: IEmail | null;
  labelColors: any;
  ...
  handleMailToTrash: Function;
  handleFolderUpdate: Function;
  handleLabelsUpdate: Function;
  handleMailReadUpdate: Function;
  formatDateToMonthShort: Function;
}

const MailDetails = (props: MailDetailsProperty) => {
  const {
    mail,
    openMail,
    dispatch,
    // labelColors, // TODO: remove this after verify the labelColors
    setOpenMail,
    updateMails,
    paginateMail,
    handleMailToTrash,
  ...
  // ** Renders Labels
  const renderLabels = (arr: LabelColor[]) => {
    if (arr && arr.length) {
      return arr.map((label: LabelColor) => (
        <Badge key={label}
          color={`light-${label}`}
          className='me-50 text-capitalize' pill
        >
          {label}
        </Badge>
      ));
  ...
  // ** Renders Attachments
  const renderAttachments = (arr: IAttachment[]) => {
    return arr.map((item: IAttachment, index: number) => {
      return (
        <a
          key={item.fileName}
          href='/'
  ...
  // ** Renders Messages
  const renderMessage = (obj: IEmail) => {
    return (
      <Card>
        <CardHeader className='email-detail-head'>
          ...
          <div className='mail-meta-item d-flex align-items-center'>
            <small className='mail-date-time text-muted'>{formatDate(String(obj.time))}</small>
  ...
  // ** Renders Replies
  const renderReplies = (arr: IEmail[]) => {
    if (arr.length && showReplies) {
  ...
  const handleFolderClick = (e: any, folder: string, id: number) => {
