...
import {ExportSelectedData} from '../interfaces/ExportSelectedData';

const initialData: ExportSelectedData[] = [
  {
    id: 1,
    name: 'Leanne Graham',
    username: 'Bret',
...
const Export = () => {
  ...
  const [filteredData, setFilteredData] = useState<ExportSelectedData[]>([]);
  const [fileFormat, setFileFormat] = useState<XLSX.BookType>('xlsx');
  ...
  const handleFilter = (e: ChangeEvent<HTMLInputElement>) => {
    const dataArr = data;
    let filteredData: ExportSelectedData[] = [];
        ...
        <ModalBody>
          ...
          <div>
            <Input
              ...
              name='customSelect'
              value={fileFormat}
              onChange={e => setFileFormat(e.target.value as XLSX.BookType)}
            >
              <option>xlsx</option>
              <option>csv</option>
              <option>txt</option>
