...
mock.onGet('/apps/todo/tasks').reply(config => {
  ...
  const includesFilter = (task: ITask2) => {
    return task.tags.includes(queryLowered) ||
            task.tags.some(tag => tag.includes(queryLowered));
  };

  const includesDueDate = (task: ITask2) => {
  ...
  // Perform sorting
  // ------------------------------------------------
  const sortTasks = (key: any) => (a: any, b: any) => {
    let fieldA;
    let fieldB;
