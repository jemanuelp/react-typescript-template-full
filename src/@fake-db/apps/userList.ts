...
mock.onPost('/apps/users/add-user').reply(config => {
    ...
    const highestValue = data.users.reduce((a, b) => {
        if (!a || !a.id) {
            return b;
        }
        if (!b || !b.id) {
    ...
    if (!highestValue) {
        throw new Error("data.users.reduce is undefined");
    }
...
// GET Updated DATA
mock.onGet('/api/users/list/data').reply((config: any) => {
    const {
        q = '',
        page = 1,
        role = null,
    ...
    const filteredData = dataToFilter.filter(
        user => (user.email.toLowerCase().includes(queryLowered) ||
                ...
                (user.billing && user.billing.toLowerCase().includes(queryLowered))) &&
